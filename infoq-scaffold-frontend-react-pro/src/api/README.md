...
- `system/user` 的个人资料接口提供当前用户 OAuth 身份列表、绑定授权地址和解绑请求；绑定由后端短期 ticket 保护，解绑是否要求当前密码以服务端返回规则为准。
- `system/message` 提供当前用户消息列表、未读数、已读、全部已读与软删除。客户端不得提供用户 ID，也不得以实时事件正文构建消息内容。
