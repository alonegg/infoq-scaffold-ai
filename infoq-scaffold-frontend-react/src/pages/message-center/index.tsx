...
export default function MessageCenterPage() {
  ...
  return (
    <Card
      title="消息中心"
      extra={
        <Button icon={<CheckOutlined />} onClick={() => void markAllRead()}>
          全部标为已读
        </Button>
      }
    >
        ...
        <List
          ...
          renderItem={(item) => (
            <List.Item
              ...
              actions={[
                !item.readTime ? (
                  <Button key="read" type="link" onClick={() => void markRead(item.messageId)}>
                    标为已读
                  </Button>
                ) : null,
            ...
            >
              <List.Item.Meta
                title={
                  <Space>
                    <Typography.Text strong={!item.readTime}>{item.title}</Typography.Text>
                    <Tag color={levelColor[item.messageLevel] || 'default'}>{item.messageLevel}</Tag>
                  </Space>
                }
                description={
                  <>
