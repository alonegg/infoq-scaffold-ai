...
export const listMessages = (pageNum = 1, pageSize = 20) =>
...
export const markMessageRead = (messageId: number) =>
  request({ url: `/system/message/${messageId}/read`, method: 'POST' });

export const markAllMessagesRead = () =>
  request({ url: '/system/message/read-all', method: 'POST' });

export const deleteMessages = (messageIds: number[]) =>
  request({ url: `/system/message/${messageIds.join(',')}`, method: 'DELETE' });
