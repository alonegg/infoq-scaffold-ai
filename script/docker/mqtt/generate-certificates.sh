...
if [[ ! -s "${truststore}" ]]; then
  keytool -importcert -noprompt -storetype PKCS12 -alias infoq-mqtt-it-ca \
...
keytool -list -storetype PKCS12 -keystore "${truststore}" \
  -storepass "${MIDDLEWARE_TRUSTSTORE_PASSWORD}" -alias infoq-mqtt-it-ca >/dev/null
